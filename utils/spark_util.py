...
class AzureConnector(object):
    def __init__(self, config):
        ...
        self.session.conf.set("fs.azure.account.auth.type.bd201stacc.dfs.core.windows.net" ,
...
class AsureReader(object):
    def __init__(self, session:pyspark.sql.session.SparkSession, data_path:str, file_format:str):
        self.session = session
        self.data_path = data_path
        self.file_format = file_format

    def read(self):
        return (self.session.read.format(self.file_format).options(header='true', inferschema='true').load(self.data_path))